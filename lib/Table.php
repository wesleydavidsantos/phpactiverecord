...
class Table
{
    ...
    */
		...
		foreach ($joins as $value)
		{
			...
			if (stripos($value,'JOIN ') === false)
			{
				if (array_key_exists($value, $this->relationships))
				{
                    ...
                    /**
                     * Depreciated Wesley David Santos - 09/07/2014 
                     * Obs.: Alteração permite definir um alias para todas as consultas, se o alias não existe então o nome da tabela se torna um alias
                    ...
                    */
                    ...
                    if( isset( $class_name::$alias_table ) )
                        $alias = $class_name::$alias_table;
                    else
                        $alias = $value;
	...
	public function options_to_sql($options)
	{
        ...
        /**
         * @author Wesley David Santos
         * @Obs Adiciona um alias a pesquisa
        */
	    $class = $this->className;
        $alias = '';
        if( isset( $class::$alias_table ) && $class::$alias_table != '' )
            $alias = '`' . $class::$alias_table . '`';
        else
            $alias = ' ' . $table;
           
		
		$sql = new SQLBuilder($this->conn, $table, $alias);
		...
		if (array_key_exists('joins',$options))
		{
			...
			if (!array_key_exists('select', $options)){
                ...
                /**
                 * @author Wesley David Santos
                 * @Obs Adiciona um alias no SELECT
                */
                if( isset( $class::$alias_table ) && $class::$alias_table != '' )
                    $options['select'] = $class::$alias_table . '.*';
                else
	...
	public function insert(&$data, $pk=null, $sequence_name=null)
	{
        ...
        $sql->ignore = $this->ignore; /* Implementado */ 
